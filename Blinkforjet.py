...
class BlinkAlgo:
    def __init__(self, max_seconds=165, show_window=True, mode="baseline"):
        ...
        self.warmup_end = 15
        self.calibration_end = 40
        self.detection_end = 160
        self.relax_end = 175


        self.checkpoints = [85, 115, 145]
        self.checkpoint_windows = {85: 45, 115: 75, 145: 105}
    ...
    def _compute_eye_state_fusion(self, smooth_ear, dark_ratio, clf_score, ear_threshold):
        ...
        dark_term = 1.0 if dark_ratio > 0.9 else (0.5 if dark_ratio > 0.85 else 0.0)
        clf_prob_closed = 1.0 / (1.0 + np.exp(-clf_score))
        clf_term = 1.0 if clf_prob_closed > 0.8 else (0.5 if clf_prob_closed > 0.70 else 0.0)
    ...
    def compute_blink_rate(self, blink_timestamps, window_sec):
        ...
        total_blinks = len(sliding_blinks)
        if not sliding_blinks:
            return {'blink_rate_per_min': 0.0, 'total_blinks': 0, 'window_duration': 0.0}
        duration = window_sec 
        blink_rate = (total_blinks / duration) * 60.0 if duration > 0 else 0.0
        return {'blink_rate_per_min': float(blink_rate), 'total_blinks': total_blinks, 'window_duration': float(duration)}
    ...
    def run(self):
        ...
        sensor_id=0,              # Use 0 for the first camera or change if you have multiple
        capture_width=1280,       # Lower width for faster processing; adjust if needed
        capture_height=720,
        display_width=640,
        display_height=360,
        framerate=30,
        ...
        try:
            while not self._stop:
                ...
                if current_phase == "relax" and elapsed_time < self.max_seconds:
                    if self.show_window:
                        cv2.putText(display, "RELAX - TEST is COMPLETE", (25, 35), cv2.FONT_HERSHEY_DUPLEX, 0.75, (0, 0, 255), 2)
                ...
                if results.multi_face_landmarks:
                    ...
                    if current_phase == "warmup":
                        ...
                        cv2.putText(
                            display, "Hello: Sit ~65cm from camera for 30s...",
                    ...
                    if current_phase == "calibration":
                        ...
                        if self.show_window:
                            ...
                            cv2.putText(display, f"Keep calm and Neutral - {max(0, self.calibration_end - elapsed_time):.1f}s remaining", (25, 55), cv2.FONT_HERSHEY_DUPLEX, 0.55, (0, 0, 0), 1)
                    ...
                    if not calibrated and elapsed_time > self.calibration_end and len(calibration_ears) > 0:
                        ...
                        dynamic_ear_threshold = max(self.ear_min_threshold, min(self.ear_max_threshold, dynamic_ear_threshold))
                        calibrated =  True
                    # ========== DETECTION PHASE ==========
                    if current_phase == "detection" and calibrated:
                        detection_elapsed = elapsed_time - self.calibration_end
                        if self.personal_baseline_blinks_per_min is None and detection_elapsed >= 60.0:
                            baseline_window_start = self.calibration_end
                            baseline_window_end = self.calibration_end + 60.0
                            baseline_blinks = [ts for ts in blink_timestamps if baseline_window_start <= (ts - start_time) <= baseline_window_end]
                            baseline_count = len(baseline_blinks)
                            self.personal_baseline_blinks_per_min = float(baseline_count)
                            print(f"[INFO] Baseline blink rate calculated (first 60s of detection): {self.personal_baseline_blinks_per_min:.2f} blinks/min")
...
if __name__ == "__main__":
    ...
    print(f"Duration: 175 seconds")
    print("Phases:")
    print("  0-15s:   Warmup (distance setup)")
    print("  15-40s:  Calibration (EAR + baseline capture)")
    print("  40-160s: Detection (checkpoints at 90s, 120s, 150s)")
    print("  160-175s: Relax (no capture)")
    print(f"{'='*70}\n")
    detector = BlinkAlgo(
        max_seconds=175,
        show_window=True,
